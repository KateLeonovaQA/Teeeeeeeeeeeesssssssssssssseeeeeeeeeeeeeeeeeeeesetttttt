...
from messaging.slack.token_resolver import (
    invalidate_cached_slack_token,
    resolve_slack_token,
)
...
class SlackClient:
    ...
    def _refresh_token(self, old_token: str) -> Optional[str]:
        ...
        Invalidates the cached token first so ``resolve_slack_token()``
        falls through to tier-2 (mcp-token) and tier-3 (token proxy)
        instead of returning the same expired token from cache.
        """
        try:
            invalidate_cached_slack_token()
