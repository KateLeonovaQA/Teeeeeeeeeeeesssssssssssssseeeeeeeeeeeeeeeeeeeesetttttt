...
def invalidate_cached_slack_token() -> None:
    """Remove bot_token from ~/.agent_settings.json so tier-2/3 are reached.

    Called by the Slack adapter when a cached token fails with
    ...
    try:
        ...
        if "bot_token" in data:
            ...
            logger.info("[Slack] invalidated cached bot_token")
    except Exception as e:
        logger.warning(f"Could not invalidate cached Slack token: {e}")
